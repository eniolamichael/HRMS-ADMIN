...
import { useHod, HodApprovalTask } from "@/contexts/hod-context";
import {
  Card,
  CardContent,
  CardHeader,
...
export default function HodApprovals() {
  ...
  const [detailsTask, setDetailsTask] = useState<HodApprovalTask | null>(null);

  const openDecision = (task: HodApprovalTask, decision: "approved" | "declined") => {
    setComment("");
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Requests routed to you as {hodName} ({departmentName})
        </p>
      </div>
      ...
      {approvalTasks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {approvalTasks.map((task) => (
            <Card key={task.id} className="border-l-4 border-l-emerald-500">
              <CardHeader>
