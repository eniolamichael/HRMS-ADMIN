...
export default function SupervisorAppraisals() {
  ...
  const pendingReviews = teamAppraisals.filter(
    (a) =>
      ...
      a.scores.some((s) => s.reviewerId === supervisorId && !s.completedAt),
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Complete assigned appraisal activities for your direct reports ({supervisorName})
        </p>
      </div>
      ...
      <Card className="bg-gradient-to-br from-amber-500 to-orange-600 border-0 text-white">
        <CardContent className="py-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-amber-100">Active Appraisal Period</p>
              <p className="font-semibold text-lg">
                {currentPeriod?.name ?? "Q4 2024 Appraisal"}
              </p>
            </div>
            <div className="ml-auto text-right">
              <p className="text-sm text-amber-100">Reviews assigned to you</p>
      ...
      {pendingReviews.length === 0 ? (
        ...
        <div className="space-y-3">
          {pendingReviews.map((a) => {
            const stage = a.scores.find((s) => s.reviewerId === supervisorId && !s.completedAt);
      ...
      {/* All direct report appraisals */}
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Direct Report Appraisals</CardTitle>
          <CardDescription>All appraisals within your reporting scope</CardDescription>
        </CardHeader>
        <CardContent>
          {teamAppraisals.length === 0 ? (
            <p className="py-8 text-center text-sm text-gray-500">No appraisals in your scope.</p>
          ) : (
